...
export class TOPSISCalculator {
  ...
  calculate(): TOPSISResult[] {
    ...
    const { positiveIdeal, negativeIdeal } =
      this.findIdealSolutions(weightedMatrix);
    ...
    const results = this.calculateTOPSISScores(
      distances,
      normalizedMatrix,
      weightedMatrix
    );
  ...
  private calculateDistances(
    ...
    return weightedMatrix.map((row) => {
      const distancePositive = Math.sqrt(
        row.reduce(
          (sum, value, index) =>
            sum + Math.pow(value - positiveIdeal[index], 2),
          0
        )
      );
      ...
      const distanceNegative = Math.sqrt(
        row.reduce(
          (sum, value, index) =>
            sum + Math.pow(value - negativeIdeal[index], 2),
          0
        )
      );
...
export async function calculateRecommendations(
  ...
  const criteria: Criteria[] = [
    ...
    {
      code: "performance",
      weight: preferences.performance_weight,
      is_benefit: true,
    },
    ...
    {
      code: "weight",
      weight: preferences.weight_importance,
      is_benefit: false,
    },
